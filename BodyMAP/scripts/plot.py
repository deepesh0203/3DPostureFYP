...
DATA_PATH = os.path.join(BASE_PATH, 'data_BP')
...
if __name__ == '__main__':
    ...
    for i in range(51,81):
        for j in range(1,46):
            ...
            name = f'{args.cover_type}_{i:03}_{j}'

            ref1, ref2 = load_RGB_images(args.cover_type, i, j)
            ...
            if is_model:
                model_str = 'Model'
                pmap = np.load(os.path.join(args.files_dir, f'{i:05}', args.cover_type, f'{j:06}_pd_pmap.npy'))
                pmap = torch.tensor(pmap).float()
                vert = np.load(os.path.join(args.files_dir, f'{i:05}', args.cover_type, f'{j:06}_pd_vertices.npy'))
            ...
            pmap_colors, vertex_colors = viz_utils.generate_colors(pmap)
            pmap_colors=pmap_colors.to("cpu").numpy()
            vert = norm(vert)
            mesh = gen_mesh(vert, pmap_colors)
            
            directory="D:\\BodyPressure\\mesh"

            if not os.path.exists(directory):
                os.makedirs(directory)

            file_path=os.path.join(directory, f'{name}.obj')
